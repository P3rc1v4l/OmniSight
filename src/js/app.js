...
const PROVIDERS_BASE = {
  ...
  // Neu in v1.1.1
  adn:          { name:'ADN',            tag:'Anime Digital Network',    url:'https://www.animedigitalnetwork.de',       color:'#0077CC', partition:'persist:adn',          quality:'HD' },
  arte:         { name:'ARTE',           tag:'Kultur & Dokumentation',   url:'https://www.arte.tv/de',                  color:'#F00000', partition:'persist:arte',         quality:'HD' },
  funk:         { name:'Funk',           tag:'Content Creator Network',  url:'https://www.funk.net',                    color:'#000000', partition:'persist:funk',         quality:'HD' },
  kika:         { name:'KiKA',           tag:'Kinder & Familie',         url:'https://www.kika.de',                     color:'#00A859', partition:'persist:kika',         quality:'HD' },
  magenta:      { name:'MagentaTV',      tag:'Telekom Streaming',        url:'https://www.magentatv.de',                color:'#E20074', partition:'persist:magenta',      quality:'4K' },
  movie2k:      { name:'Movie2k',        tag:'Filme & Serien',           url:'https://movie2k.ch/',                     color:'#FF6B35', partition:'persist:movie2k',      quality:'HD' },
  spotify:      { name:'Spotify',        tag:'Musik & Podcasts',         url:'https://open.spotify.com',                color:'#1DB954', partition:'persist:spotify',      quality:'-',  bgAudio:true },
  waipu:        { name:'Waipu.tv',       tag:'Live-TV & Mediathek',      url:'https://www.waipu.tv',                    color:'#00B4D8', partition:'persist:waipu',        quality:'HD' },
  wow:          { name:'WOW',            tag:'Sky ohne Abo',             url:'https://www.wowtv.de',                    color:'#00A3E0', partition:'persist:wow',          quality:'HD' },
};

// Custom-Provider und Hilfsfunktion
let customProviders = {};
...
function getProfilePartition(id) {
  const p = PROVIDERS()[id];
  const basePart = p?.partition?.replace('persist:', '') || id;
  return `persist:${activeProfileId}_${basePart}`;
}
...
async function init() {
  ...
  document.getElementById('btn-sort-alpha')?.addEventListener('click', () => {
    settings.sortAlpha = !settings.sortAlpha;
    autoSave();
    buildProviderGrid();
...
function startSessionAutoRefresh() {
  window.electronAPI.refreshSessionsNow(activeProfileId); // sofort einmal
  clearInterval(sessionRefreshTimer);
  sessionRefreshTimer = setInterval(()=>window.electronAPI.refreshSessionsNow(activeProfileId), 60000);
}
...
function extractYtId(q) {
  ...
  for (const p of patterns) { const m = q.match(p); if (m) return m[1]; }
  return null;
}
  ...
  input?.addEventListener('input',()=>{
    ...
    if(ytId){
      dd.innerHTML='<div class="search-dd-section">YouTube</div>' +
        '<div class="search-dd-item" id="yt-dd-item" style="cursor:pointer">' +
        '<img src="https://img.youtube.com/vi/'+ytId+'/mqdefault.jpg" style="width:80px;height:46px;border-radius:5px;object-fit:cover;flex-shrink:0" onerror="this.style.display=\'none\'"/>' +
        '<div class="search-dd-info"><div class="search-dd-title">YouTube Video abspielen</div>' +
        '<div class="search-dd-meta">Direkt in OmniSight \u00f6ffnen</div></div></div>';
      dd.style.display='block';
      document.getElementById('yt-dd-item')?.addEventListener('click',()=>{
        dd.style.display='none';
        openProviderAtUrl('youtube','https://www.youtube.com/watch?v='+ytId,'YouTube',getProfilePartition('youtube'));
      });
      return;
    }
...
function openProviderAtUrl(id, url, name, partitionOverride) {
  const partition = partitionOverride || getProfilePartition(id);
...
function setupStreamControls() {
  ...
  document.getElementById('btn-logout-provider')?.addEventListener('click',()=>{
    ...
    window.electronAPI.clearProviderSession(currentProvider);if(currentWebview)currentWebview.loadURL(p.url);window.electronAPI.refreshSessionsNow(activeProfileId);
  });
}
...
function setupSidebarCrunchyroll() {
  ...
  toggle.addEventListener('click', () => {
    ...
    if (!open && !content.dataset.loaded) { content.dataset.loaded = '1'; loadSidebarCR(content); }
  });
}

async function loadSidebarCR(container) {
  container.innerHTML = '<div style="font-size:11px;color:var(--tx3);padding:8px;text-align:center">Lädt…</div>';
  try {
    const resp = await window.electronAPI.getUpcoming(1);
    const anime = (resp.anime || []).slice(0, 10);
    if (!anime.length) { container.innerHTML = '<div style="font-size:11px;color:var(--tx3);padding:8px;text-align:center">Keine Daten</div>'; return; }
    container.innerHTML = '';
    anime.forEach(item => {
      const title = item.title || item.name || 'Unbekannt';
      ...
      const el = document.createElement('div');
      el.style.cssText = 'display:flex;align-items:center;gap:7px;padding:5px 6px;border-radius:6px;cursor:pointer;transition:background .14s;font-size:11px;color:var(--tx2)';
      el.innerHTML = `${poster ? `<img src="${poster}" style="width:28px;height:40px;object-fit:cover;border-radius:3px;flex-shrink:0" loading="lazy"/>` : ''}
        <div style="flex:1;min-width:0">
          <div style="font-weight:600;font-size:11px;color:var(--tx);white-space:nowrap;overflow:hidden;text-overflow:ellipsis">${title}</div>
          <div style="font-size:9px;color:var(--acc);font-weight:600;margin-top:1px">📅 ${dateStr}</div>
        </div>`;
      el.onmouseenter = () => el.style.background = 'var(--bgc)';
      el.onmouseleave = () => el.style.background = '';
      el.addEventListener('click', () => showDetailPopup(item.id, item.title ? 'movie' : 'tv', title));
      container.appendChild(el);
    });
    const cta = document.createElement('div');
    cta.style.cssText = 'text-align:center;padding:6px;font-size:11px;color:var(--acc);font-weight:600;cursor:pointer;border-top:1px solid var(--bor);margin-top:4px';
    cta.textContent = '▶ Crunchyroll öffnen';
    cta.addEventListener('click', () => openProvider('crunchyroll'));
    container.appendChild(cta);
  } catch { container.innerHTML = '<div style="font-size:11px;color:var(--danger);padding:8px">Fehler</div>'; }
}

document.addEventListener('DOMContentLoaded', () => {
  // Meta-Tracking für versteckte Achievements
  try {
    const profileId = 'default'; // wird nach init() korrekt gesetzt
    const meta = JSON.parse(localStorage.getItem('achMeta_default') || '{}');
    meta.appStarts = (meta.appStarts || 0) + 1;
    localStorage.setItem('achMeta_default', JSON.stringify(meta));
  } catch {}
  init();
});

// ════════════════════════════════
// ACHIEVEMENTS (25 Stück, 4 Kategorien)
// ════════════════════════════════
...
const ACHIEVEMENTS = [
  // ── Streamzeit ──
  {id:'first_stream',   cat:'stream',  icon:'🎬', name:{de:'Erster Stream',    en:'First Stream'},      desc:{de:'Ersten Stream gestartet',         en:'Started first stream'},       check:s=>Object.values(s).some(v=>(v.total||v||0)>0)},
  {id:'hour_1',         cat:'stream',  icon:'⏰', name:{de:'1 Stunde',         en:'1 Hour'},             desc:{de:'1h insgesamt gestreamt',           en:'1h streamed total'},           check:s=>totalSecs(s)>=3600},
  {id:'hour_5',         cat:'stream',  icon:'🕔', name:{de:'5 Stunden',        en:'5 Hours'},            desc:{de:'5h insgesamt gestreamt',           en:'5h streamed total'},           check:s=>totalSecs(s)>=18000},
  {id:'hour_10',        cat:'stream',  icon:'🕐', name:{de:'10 Stunden',       en:'10 Hours'},           desc:{de:'10h insgesamt gestreamt',          en:'10h streamed total'},          check:s=>totalSecs(s)>=36000},
  {id:'hour_50',        cat:'stream',  icon:'🏅', name:{de:'50 Stunden',       en:'50 Hours'},           desc:{de:'50h insgesamt gestreamt',          en:'50h streamed total'},          check:s=>totalSecs(s)>=180000},
  {id:'hour_100',       cat:'stream',  icon:'💯', name:{de:'100 Stunden',      en:'100 Hours'},          desc:{de:'100h insgesamt gestreamt',         en:'100h streamed total'},         check:s=>totalSecs(s)>=360000},
  {id:'hour_500',       cat:'stream',  icon:'🏆', name:{de:'500 Stunden',      en:'500 Hours'},          desc:{de:'500h insgesamt gestreamt',         en:'500h streamed total'},         check:s=>totalSecs(s)>=1800000},
  ...
  {id:'monday_start',   cat:'stream',  icon:'📅', name:{de:'Wochenstart',      en:'Week Starter'},       desc:{de:'An einem Montag gestreamt',        en:'Streamed on Monday'},          check:s=>Object.values(s).some(v=>(v.byDay?.[1]||0)>0)},
  // ── Anbieter ──
  {id:'netflix_1h',     cat:'provider',icon:'🍿', name:{de:'Netflix Fan',      en:'Netflix Fan'},        desc:{de:'1h Netflix gestreamt',             en:'1h on Netflix'},               check:s=>getSecs(s,'netflix')>=3600},
  {id:'youtube_5h',     cat:'provider',icon:'▶️', name:{de:'YouTube Addict',   en:'YouTube Addict'},     desc:{de:'5h YouTube gestreamt',             en:'5h on YouTube'},               check:s=>getSecs(s,'youtube')>=18000},
  {id:'anime_fan',      cat:'provider',icon:'⛩️', name:{de:'Anime-Fan',        en:'Anime Fan'},          desc:{de:'1h Crunchyroll gestreamt',         en:'1h on Crunchyroll'},           check:s=>getSecs(s,'crunchyroll')>=3600},
  {id:'anime_master',   cat:'provider',icon:'🐉', name:{de:'Anime-Meister',    en:'Anime Master'},       desc:{de:'10h Crunchyroll gestreamt',        en:'10h on Crunchyroll'},          check:s=>getSecs(s,'crunchyroll')>=36000},
  {id:'multi_provider', cat:'provider',icon:'🌐', name:{de:'Viel-Streamer',    en:'Multi-Streamer'},     desc:{de:'5 Anbieter genutzt',               en:'Used 5 providers'},            check:s=>Object.values(s).filter(v=>totalSecs({x:v})>0).length>=5},
  {id:'all_providers',  cat:'provider',icon:'🎯', name:{de:'Komplett-Streamer',en:'Full Streamer'},       desc:{de:'10 Anbieter genutzt',              en:'Used 10 providers'},           check:s=>Object.values(s).filter(v=>totalSecs({x:v})>0).length>=10},
  {id:'sport_fan',      cat:'provider',icon:'⚽', name:{de:'Sport-Fan',        en:'Sports Fan'},          desc:{de:'1h DAZN gestreamt',                en:'1h on DAZN'},                  check:s=>getSecs(s,'dazn')>=3600},
  {id:'music_fan',      cat:'provider',icon:'🎵', name:{de:'Musik-Fan',        en:'Music Fan'},           desc:{de:'1h Spotify gestreamt',             en:'1h on Spotify'},               check:s=>getSecs(s,'spotify')>=3600},
  {id:'twitch_fan',     cat:'provider',icon:'🎮', name:{de:'Twitch-Fan',       en:'Twitch Fan'},          desc:{de:'2h Twitch gestreamt',              en:'2h on Twitch'},                check:s=>getSecs(s,'twitch')>=7200},
  // ── Besonders ──
  {id:'binge_day',      cat:'special', icon:'🛋️', name:{de:'Binge-Tag',        en:'Binge Day'},           desc:{de:'4h an einem Tag gestreamt',        en:'4h in one day'},               check:s=>Object.values(s).some(v=>v.byDay&&Math.max(...v.byDay)>=14400)},
  {id:'early_bird',     cat:'special', icon:'🌅', name:{de:'Frühaufsteher',    en:'Early Bird'},          desc:{de:'3h montags insgesamt gestreamt',   en:'3h on Mondays total'},         check:s=>Object.values(s).reduce((a,v)=>a+(v.byDay?.[1]||0),0)>=10800},
  // ── Versteckt ──
  ...
  {id:'hid_settings50', cat:'hidden',  icon:'⚙️', name:{de:'Einstellungs-Freak',en:'Settings Freak'},     desc:{de:'Einstellungen 50× geöffnet',      en:'Settings opened 50×'},         check:(_,m)=>(m?.settingsOpens||0)>=50,  hidden:true},
  {id:'hid_1000h',      cat:'hidden',  icon:'👑', name:{de:'Lebenswerk',       en:"Life's Work"},          desc:{de:'1000h insgesamt gestreamt',        en:'1000h streamed total'},        check:s=>totalSecs(s)>=3600000,          hidden:true},
  ...
  {id:'hid_allprovider',cat:'hidden',  icon:'🔬', name:{de:'Alles-Tester',     en:'All-Tester'},           desc:{de:'20 Standardanbieter genutzt',     en:'Used all 20 default providers'},check:s=>Object.values(s).filter(v=>totalSecs({x:v})>0).length>=20, hidden:true},
];

function totalSecs(s){return Object.values(s).reduce((a,v)=>a+(v?.total||v||0),0);}
function getSecs(s,id){const v=s[id];return v?.total||v||0;}

function trackMeta(key){
  try{
    const k='achMeta_'+activeProfileId;
    const meta=JSON.parse(localStorage.getItem(k)||'{}');
    meta[key]=(meta[key]||0)+1;
    // Mitternacht prüfen
    if(key==='stream'&&new Date().getHours()<4)meta.midnightStreams=(meta.midnightStreams||0)+1;
...
async function checkAchievements(){
  try{
    ...
    ACHIEVEMENTS.forEach(ach=>{
      ...
      let ok=false;
      try{ok=ach.check(stats,meta);}catch{}
      if(ok){
        earned.add(ach.id);newOnes=true;
        const name=ach.name[lang]||ach.name.de;
        const desc=ach.desc[lang]||ach.desc.de;
        setTimeout(()=>window.electronAPI.showNotification('🏆 Achievement!',ach.icon+' '+name+': '+desc),300);
      }
    });
...
function buildAchievements(){
  ...
  cats.forEach(cat=>{
    ...
    const label=ACHIEVEMENT_CATS[cat][lang]||ACHIEVEMENT_CATS[cat].de;
    const sec=document.createElement('div');sec.style.cssText='margin-bottom:18px';
    sec.innerHTML='<div style="font-size:11px;font-weight:700;color:var(--tx2);text-transform:uppercase;letter-spacing:.1em;margin-bottom:8px">'+label+'</div>';
    const grid=document.createElement('div');grid.style.cssText='display:flex;flex-wrap:wrap;gap:6px';
    if(cat==='hidden'){
      const done=list.filter(a=>earned.has(a.id));
      const locked=list.filter(a=>!earned.has(a.id));
      done.forEach(ach=>{
        const c=document.createElement('div');
        c.style.cssText='display:flex;align-items:center;gap:8px;padding:8px 12px;background:var(--accg);border:1px solid var(--acc);border-radius:var(--r-sm);font-size:12px;color:var(--acc);cursor:default';
        c.title=ach.desc[lang]||ach.desc.de;
        c.innerHTML='<span style="font-size:18px">'+ach.icon+'</span><div><div style="font-weight:600">'+esc(ach.name[lang]||ach.name.de)+'</div><div style="font-size:10px;opacity:.7">'+esc(ach.desc[lang]||ach.desc.de)+'</div></div>';
        grid.appendChild(c);
      });
      if(locked.length){
        const hint=document.createElement('div');
        hint.style.cssText='display:flex;align-items:center;gap:8px;padding:8px 12px;background:var(--bgc);border:1px solid var(--bor);border-radius:var(--r-sm);font-size:12px;color:var(--tx3)';
        hint.innerHTML='🔒 '+locked.length+'× Versteckte Achievements – durch Spielen freischalten';
        grid.appendChild(hint);
      }
    }else{
      list.forEach(ach=>{
        const ok=earned.has(ach.id);
        const c=document.createElement('div');
        c.style.cssText='display:flex;align-items:center;gap:8px;padding:8px 12px;background:'+(ok?'var(--accg)':'var(--bgc)')+';border:1px solid '+(ok?'var(--acc)':'var(--bor)')+';border-radius:var(--r-sm);font-size:12px;color:'+(ok?'var(--acc)':'var(--tx2)')+';opacity:'+(ok?1:.55)+';cursor:default;transition:opacity .2s';
        c.title=ach.desc[lang]||ach.desc.de;
        c.innerHTML='<span style="font-size:18px">'+ach.icon+'</span><div><div style="font-weight:600;font-size:12px">'+esc(ach.name[lang]||ach.name.de)+'</div><div style="font-size:10px;opacity:.7;max-width:160px;line-height:1.3">'+esc(ach.desc[lang]||ach.desc.de)+'</div></div>';
        grid.appendChild(c);
      });
    }
...
// ════════════════════════════════
// STATS ERWEITERT (Top3 + Wochentage + CR-Kalender + Achievements)
// ════════════════════════════════
async function buildStatsView() {
  const stats=await window.electronAPI.getStreamStats(activeProfileId);
  const content=document.getElementById('stats-content');
  ...
  const entries=Object.entries(stats)
    .map(([id,v])=>({id,secs:v?.total||v||0}))
    .filter(e=>e.secs>0)
    .sort((a,b)=>b.secs-a.secs);

  if(!entries.length){
    content.innerHTML='<div style="color:var(--tx2);font-size:13px;padding:10px 0">Noch keine Stream-Daten gesammelt.</div>';
  }else{
    const maxSecs=entries[0].secs;
    const top=document.createElement('div');
    top.innerHTML='<h3 style="font-family:var(--font-d);font-size:15px;color:var(--tx);margin-bottom:12px">⏱ Meiste Streamzeit (Top 3)</h3>';
    entries.slice(0,3).forEach(({id,secs})=>{
      const p=PROVIDERS()[id];if(!p)return;
      const hours=(secs/3600).toFixed(1);
      const pct=Math.round((secs/maxSecs)*100);
      const w=document.createElement('div');w.className='stats-bar-wrap';
      w.innerHTML='<div class="stats-bar-label"><span>'+esc(p.name)+'</span><span>'+hours+'h</span></div><div class="stats-bar"><div class="stats-bar-fill" style="width:'+pct+'%;background:'+p.color+'"></div></div>';
      top.appendChild(w);
    });
    ...
    entries.forEach(({id})=>{
      const v=stats[id];
      if(v?.byDay)v.byDay.forEach((s,i)=>dayTotals[i]+=s);
    });
    const maxDay=Math.max(...dayTotals,1);
    const dw=document.createElement('div');dw.style.cssText='margin-top:20px';
    dw.innerHTML='<h3 style="font-family:var(--font-d);font-size:15px;color:var(--tx);margin-bottom:12px">📅 Wochentage</h3>';
    days.forEach((d,i)=>{
      const pct=Math.round((dayTotals[i]/maxDay)*100);
      const h=(dayTotals[i]/3600).toFixed(1);
      const bar=document.createElement('div');bar.className='stats-bar-wrap';
      bar.innerHTML='<div class="stats-bar-label"><span>'+d+'</span><span>'+h+'h</span></div><div class="stats-bar"><div class="stats-bar-fill" style="width:'+pct+'%;background:var(--acc)"></div></div>';
      dw.appendChild(bar);
    });
    content.appendChild(dw);
  }

  // Crunchyroll Kalender in Statistiken
  const crSection=document.createElement('div');crSection.style.cssText='margin-top:24px';
  crSection.innerHTML='<h3 style="font-family:var(--font-d);font-size:15px;color:var(--tx);margin-bottom:12px">⛩️ Crunchyroll Release-Kalender</h3>';
  const crGrid=document.createElement('div');crSection.appendChild(crGrid);
  content.appendChild(crSection);
  loadCrunchyrollCalendarStats(crGrid);

  // Achievements
  const achEl=document.getElementById('achievements-content');
  if(achEl){await checkAchievements();buildAchievements();}
}

async function loadCrunchyrollCalendarStats(container){
  container.innerHTML='<div style="color:var(--tx2);font-size:13px">Lädt…</div>';
  try{
    const resp=await window.electronAPI.getUpcoming(1);
    const anime=(resp.anime||[]).slice(0,24);
    if(!anime.length){container.innerHTML='<div style="color:var(--tx2);font-size:13px">Keine kommenden Anime-Releases.</div>';return;}
    container.innerHTML='';
    ...
    Object.entries(byDate).sort(([a],[b])=>a.localeCompare(b)).forEach(([date,items])=>{
      const label=date!=='Unbekannt'?new Date(date).toLocaleDateString('de-DE',{weekday:'long',day:'2-digit',month:'long'}):'Datum unbekannt';
      const sec=document.createElement('div');sec.style.cssText='margin-bottom:16px';
      sec.innerHTML='<div style="font-family:var(--font-d);font-size:13px;font-weight:700;color:var(--acc);margin-bottom:8px;padding-bottom:4px;border-bottom:1px solid var(--bor)">'+label+'</div>';
      const g=document.createElement('div');g.style.cssText='display:grid;grid-template-columns:repeat(auto-fill,minmax(120px,1fr));gap:8px';
      items.forEach(item=>{
        ...
        card.innerHTML=(poster?'<img src="'+poster+'" style="width:100%;aspect-ratio:2/3;object-fit:cover;display:block" loading="lazy"/>':'')+'<div style="padding:6px 8px"><div style="font-size:11px;font-weight:600;color:var(--tx);line-height:1.3;display:-webkit-box;-webkit-line-clamp:2;-webkit-box-orient:vertical;overflow:hidden">'+esc(title)+'</div></div>';
    ...
    const btn=document.createElement('button');
    btn.className='pick-btn';btn.style.cssText='margin-top:8px;cursor:pointer';
    btn.textContent='▶ Auf Crunchyroll ansehen';
    btn.addEventListener('click',()=>openProvider('crunchyroll'));
    container.appendChild(btn);
  }catch(e){container.innerHTML='<div style="color:var(--danger);font-size:13px">Fehler: '+e.message+'</div>';}
}

// ════════════════════════════════
// WATCHLIST – Rechtsklick Kategorie + Auto-Entfernen nach Popup
// ════════════════════════════════
function buildWatchlistV2(cat){
  const catArg=cat||document.querySelector('#wl-switcher .wl-cat-btn.active')?.dataset.cat||'all';
  const sort=document.getElementById('wl-sort')?.value||'alpha';
  const content=document.getElementById('watchlist-content');if(!content)return;
  ...
  items.forEach(item=>{
    ...
    card.querySelector('.wl-card-remove').addEventListener('click',e=>{
      ...
      settings.watchlist=watchlist;autoSave();
      card.style.opacity='0';card.style.transform='scale(.9)';card.style.transition='all .25s';
      setTimeout(()=>buildWatchlistV2(catArg),260);
    });
    // Klick → Detail-Popup → Auto-Entfernen wenn abgewählt
    card.addEventListener('click',async()=>{
      ...
      const check=setInterval(()=>{
        if(document.getElementById('detail-overlay').style.display==='none'){
          clearInterval(check);
          if(!watchlist.find(w=>w.id===item.id)){buildWatchlistV2(catArg);}
        }
      },300);
    });
    // Rechtsklick → Kategorie wechseln
    card.addEventListener('contextmenu',e=>{
      e.preventDefault();
      document.querySelectorAll('.wl-ctx-menu').forEach(m=>m.remove());
      const menu=document.createElement('div');
      menu.className='wl-ctx-menu';
      menu.style.cssText='position:fixed;left:'+e.clientX+'px;top:'+e.clientY+'px;background:var(--bg2);border:1px solid var(--borh);border-radius:var(--r-sm);z-index:3000;min-width:160px;box-shadow:0 8px 24px rgba(0,0,0,.5);padding:4px 0';
      [{v:'movie',l:'🎬 Film'},{v:'tv',l:'📺 Serie'},{v:'anime',l:'⛩️ Anime'}].forEach(c=>{
        const btn=document.createElement('button');
        btn.style.cssText='display:block;width:100%;padding:8px 14px;border:none;background:transparent;color:var(--tx);font-size:13px;cursor:pointer;text-align:left';
        btn.innerHTML=(c.v===item.mediaType?'✓ ':'')+c.l;
        ...
        btn.addEventListener('click',()=>{
          ...
          menu.remove();buildWatchlistV2(catArg);
        });
...
// Watchlist-Switcher Buttons (Text-Style)
function setupWatchlistSwitcher(){
  const sw=document.getElementById('wl-switcher');if(!sw)return;
  sw.querySelectorAll('.wl-cat-btn').forEach(btn=>{
    btn.addEventListener('click',()=>{
      sw.querySelectorAll('.wl-cat-btn').forEach(b=>b.classList.remove('active'));
      btn.classList.add('active');
      buildWatchlistV2(btn.dataset.cat);
    });
  });
}

// ════════════════════════════════
// KARTEN-EDITOR: Reset + Logo + Einzelne Provider wiederherstellen
// ════════════════════════════════
function setupCardEditorExtra(){
  // Reset-Button
  document.getElementById('card-ed-reset')?.addEventListener('click',()=>{
    const id=imgEditorState?.providerId||window._cardEditorId;
    if(!id)return;
    delete(settings.cardImages||{})[id];
    delete(settings.cardImageOffsets||{})[id];
    delete(settings.cardBgColors||{})[id];
    delete(settings.cardBgOpacity||{})[id];
    delete(settings.cardCustomNames||{})[id];
    delete(settings.cardCustomTags||{})[id];
    delete(settings.cardLogos||{})[id];
    autoSave();buildProviderGrid();
    showToast('Karte zurückgesetzt.');
  });
  // Logo-Picker
  document.getElementById('card-ed-logo-pick')?.addEventListener('click',async()=>{
    const id=window._cardEditorId;if(!id)return;
    const url=await window.electronAPI.pickImage('logo_'+id);
    if(url){settings.cardLogos=settings.cardLogos||{};settings.cardLogos[id]=url;autoSave();}
  });
  document.getElementById('card-ed-logo-remove')?.addEventListener('click',()=>{
    const id=window._cardEditorId;if(!id)return;
    delete(settings.cardLogos||{})[id];autoSave();
  });
  ...
  document.getElementById('btn-restore-single-open')?.addEventListener('click',()=>{
    ...
    deleted.forEach(id=>{
      ...
      const btn=document.createElement('button');
      btn.className='pick-btn';btn.style.cssText='width:100%;text-align:left;margin-bottom:4px;cursor:pointer';
      btn.textContent='↺ '+p.name;
      btn.addEventListener('click',()=>{
        settings.deletedProviders=(settings.deletedProviders||[]).filter(d=>d!==id);
        autoSave();buildProviderGrid();
...
// ════════════════════════════════
// UHR: Analog/Digital + Sekunden + 100%=deaktiviert
// ════════════════════════════════
function setupClockV2(){
  clearInterval(window._clockIntervalV2);
  const widget=document.getElementById('clock-widget');
  ...
  if(!clk.enabled){widget.style.display='none';return;}
  const pos=clk.position||{x:16,y:52};
  widget.style.cssText='position:fixed;z-index:9000;pointer-events:all;user-select:none;left:'+pos.x+'px;top:'+pos.y+'px;right:auto;bottom:auto;background:none;border:none;padding:0;color:'+(clk.color||'#ff3b30')+';font-size:'+(clk.size||22)+'px;opacity:'+(1-(clk.opacity??0.5))+';font-family:var(--font-d);font-weight:700;letter-spacing:.08em';
  widget.style.display='block';
  const showSeconds=!!clk.showSeconds;
  if(clk.type==='analog'){
    function drawAnalog(){
      const n=new Date(),h=n.getHours()%12,m=n.getMinutes(),s=n.getSeconds();
      const sz=Math.max(clk.size||22,18),r=sz*1.8;
      ...
      timeEl.innerHTML='<svg width="'+r*2+'" height="'+r*2+'" viewBox="0 0 '+r*2+' '+r*2+'" style="display:block"><circle cx="'+r+'" cy="'+r+'" r="'+(r-2)+'" fill="none" stroke="'+c+'" stroke-width="1.5" opacity=".4"/><line x1="'+r+'" y1="'+r+'" x2="'+(r+Math.cos(ha)*r*.55)+'" y2="'+(r+Math.sin(ha)*r*.55)+'" stroke="'+c+'" stroke-width="2.5" stroke-linecap="round"/><line x1="'+r+'" y1="'+r+'" x2="'+(r+Math.cos(ma)*r*.8)+'" y2="'+(r+Math.sin(ma)*r*.8)+'" stroke="'+c+'" stroke-width="1.8" stroke-linecap="round"/>'+(showSeconds?'<line x1="'+r+'" y1="'+r+'" x2="'+(r+Math.cos(sa)*r*.85)+'" y2="'+(r+Math.sin(sa)*r*.85)+'" stroke="'+c+'" stroke-width=".8" stroke-linecap="round" opacity=".7"/>':'')+'<circle cx="'+r+'" cy="'+r+'" r="2" fill="'+c+'"/></svg>';
    }
    drawAnalog();
    window._clockIntervalV2=setInterval(drawAnalog,showSeconds?1000:10000);
  }else{
    function tick(){
      const n=new Date();
      const pad=x=>String(x).padStart(2,'0');
      timeEl.textContent=pad(n.getHours())+':'+pad(n.getMinutes())+(showSeconds?':'+pad(n.getSeconds()):'');
    }
    tick();
    window._clockIntervalV2=setInterval(tick,1000);
  }
}

// Clock-Typ-Buttons
function setupClockTypeBtns(){
  document.querySelectorAll('.clock-type-btn').forEach(btn=>{
    btn.addEventListener('click',()=>{
      ...
      settings.clock.type=btn.dataset.clockType||'digital';
      autoSave();setupClockV2();
    });
  });
  document.getElementById('clock-show-seconds')?.addEventListener('change',e=>{
    settings.clock=settings.clock||{};
    settings.clock.showSeconds=e.target.checked;
    autoSave();setupClockV2();
  });
  // 100% Transparenz → auto-deaktivieren
  document.getElementById('clock-opacity')?.addEventListener('change',e=>{
    if(parseInt(e.target.value)>=100){
      settings.clock=settings.clock||{};
      settings.clock.enabled=false;
      settings.clock.opacity=1;
...
// Settings öffnen: trackMeta + Drag nur im clock-Tab
function openSettingsV2(){
  document.getElementById('settings-panel')?.classList.add('open');
  document.getElementById('settings-overlay')?.classList.add('open');
...
function rebuildExtraDomains(){
  const all=[...new Set(Object.values(pluginDomainStore).flat())];
  window.electronAPI.applyExtraAdDomains(all);
  const el=document.getElementById('plugin-domain-count');
  if(el)el.textContent=all.length;
}

// ════════════════════════════════
// HOOK: alle neuen Features an init anhängen
// ════════════════════════════════
(function hookInit(){
  ...
  if(typeof origShowView==='function'){
    window.showView=function(v){
      origShowView(v);
      if(v==='watchlist'){setupWatchlistSwitcher();buildWatchlistV2();}
      if(v==='stats'){buildStatsView();}
    };
  }
})();

// Init-Hook: nach DOMContentLoaded extra Setup
document.addEventListener('DOMContentLoaded',()=>{
  setTimeout(()=>{
    setupCardEditorExtra();
    setupClockTypeBtns();
    setupWatchlistSwitcher();
    // Analog/Sekunden syncen
    const clk=settings.clock||{};
    ...
    // Settings-Button überschreiben
    const origSetupSettings=document.getElementById('btn-settings');
    origSetupSettings?.addEventListener('click',()=>trackMeta('settingsOpens'),{capture:true});
    // Statistiken-Link in Sidebar
    document.querySelector('[data-view="stats"]')?.addEventListener('click',()=>{
      setTimeout(buildAchievements,100);
    });
    // Sort-Alpha initial
